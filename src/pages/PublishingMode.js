...
function validateStakingAmount(number) {
  if (number < 11) {
    return {
...
class PublishingMode extends React.Component {
  state = {
    amountStaked: 1
  };
  ...
  render() {
    ...
    const tips =
      "Minium amount to stake on the platform is set at $1 DAI. This is to incentivize readers";
    ...
    return (
      <Layout style={{ padding: "24px 20px", background: "#fff" }}>
        ...
        <Form>
          <Form.Item
            {...formItemLayout}
            label="Stake DAI to incentivize readers"
            help={tips}
          >
            <InputNumber
              min={1}
